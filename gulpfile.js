...
gulp.task('all-scripts', ['convert2html'], () =>
{
...
gulp.task('build', ['all-styles', 'vendorsjs', 'all-scripts']);

gulp.task('watch', ()=>
{

    gulp.watch(config.files.jade, ['all-scripts']);
    gulp.watch(config.files.js, ['all-scripts']);
    gulp.watch(config.files.sass, ['all-styles']);
});
